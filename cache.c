...
int main(int argc, char * argv[])
{
  ...
  printf("Total number of blocks in the cache: %ld\n", find_total_num_blocks());
  ...
  {
    printf("address = %0xlx  TAG = %u   SET = %u   OFFSET = %u\n",
	    address,
	    find_tag(address),
	    find_set(address),
	    find_offset(address)
...
unsigned long find_tag(unsigned long address){
  ...
  return address >>( num_bits_set() + num_bits_offset());
...
unsigned long find_offset(unsigned long address){
  ...
  return  (address <<(num_bits_tag()+num_bits_set()))>>(num_bits_tag()+num_bits_set());
